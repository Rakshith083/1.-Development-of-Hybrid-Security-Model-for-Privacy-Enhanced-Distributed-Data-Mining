...
def clear():
    ...
    UserorgDataList.clear()
    UserROtdatList.clear()
...
def upload():
    ...
    return render_template("file_upload_form.html",link="/Showdata")  
...
def success():  
    if request.method == 'POST':  
        ...
        f.save(f.filename)
        upfile.append(f.filename)
        print(upfile)
        return render_template('loader.html',link="/Showdata")

@app.route('/Showdata')
def showdata():
    data = pd.read_csv(upfile[0])
    ...
    while k<cnt:
        ...
        #plt.show() 
        plt.savefig(upfile[0]+'_Alpha'+str(k)+'.png')
        alpha_graph.append(upfile[0]+'_Alpha'+str(k)+'.png')
        ...
        plt.legend(loc='upper left')
        #plt.show()
        plt.savefig(upfile[0]+'_Beta'+str(k)+'.png')
        beta_graph.append(upfile[0]+'_Beta'+str(k)+'.png')
...
def RotateAngle():
    ...
    return render_template("RotatedTestData.html",len=len,cols=TstCols,r=rotateded_test_data,round=round,l=len(rotateded_test_data),z=zip,a=accuracy)

     
UserorgDataList=[]
UserROtdatList=[]

@app.route("/newEntery",methods=['Post'])
def entry():
    UserorgData={}
    for i in cols[0:len(cols)-1]:
        UserorgData[i]=float(request.form[i])
    
    orgdf=pd.DataFrame(UserorgData,index=[0])
    
    for row in orgdf.values.tolist():
        UserorgDataList.append(row)
    
    print(pd.DataFrame(UserorgDataList,columns=cols[0:len(cols)-1]))


    print(pd.DataFrame(orgdf))

    length=len(orgdf.columns)

    x_train=pd.DataFrame(Origional_data,columns=cols)
    
    for i in range(length):
        mx = x_train[cols[i]].max()
        mn = x_train[cols[i]].min()

        orgdf[cols[i]][0] = 5 * (orgdf[cols[i]] - mn) / (mx - mn)
    ...
    for row in UserROtdat.values.tolist():
        UserROtdatList.append(row)

    print("*******************Rotated input DATA*******************\n",pd.DataFrame(UserROtdatList,columns=cols[:len(cols)-1]))
    
    return render_template("loader.html",link="/predict")

@app.route('/predict')
def predict():
    length=len(cols[0:len(cols)-1])
    ...
    x_test = pd.DataFrame(UserROtdatList,columns=cols[:len(cols)-1]).round(decimals=1)
    ...
    print(UserorgDataList[0])
    print(UserROtdatList[0])

    return render_template('UserData.html',o=UserorgDataList[0],r=UserROtdatList[0],p=y_pred,zip=zip,c=cols[0:len(cols)-1])
